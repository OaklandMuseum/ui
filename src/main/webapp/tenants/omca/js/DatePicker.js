...
(function ($, fluid) {
    ...
    cspace.datePicker.postInit = function (that) {
        ...
        // OMCA. Reformat the date displayed so that it leaves off the hour:secconds
        var getTimeStamp = /^(.*)T.*$/;
        if(that.container.context.value && that.container.context.value.length) {
            var dateMatch = that.container.context.value.match(getTimeStamp);
            if (dateMatch && dateMatch.length > 1) {
                // apply the new value to the field but don't change it
