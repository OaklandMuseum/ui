...
(function ($, fluid) {
    ...
    cspace.advancedSearch.search = function (searchEvent, keywordModel, fieldsModel) {
        ...
        if (fieldsModel) {
            ...
            // dumb hack to made sure boolean values are not used as strings
            $.each(fieldsModel, function(k,v){
                if (v && v.length == 1 && (v[0] === "true" || v[0] === "false")){
                    var boolVal = false;
                    if (v[0] === "true") {
                        boolVal = true;
                    }
                    fieldsModel[k] = boolVal;
                }
            });
            // copy over boolean value rather than string
